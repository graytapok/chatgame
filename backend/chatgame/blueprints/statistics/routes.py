...
def get_current_user_statistics():
    statistics = utils.get_user_total_statistics(str(current_user.id))
...
def get_user_statistics(user_id: str):
    statistics = utils.get_user_total_statistics(user_id)
...
def get_friends_leaderboard():
    leaderboard = utils.get_friends_leaderboard(current_user)
    return [LeaderboardUserDto.model_validate(i) for i in leaderboard]